...
if( empty($config['label_show_product_details']) ){
	$config['label_show_product_details'] = __('Show product details', 'woolentor-pro') ;
}

if( empty($config['label_compare']) ){
	$config['label_compare'] = __('Compare', 'woolentor-pro') ;
}
...
if( empty($config['label_already_added_to_wishlist']) ){
	$config['label_already_added_to_wishlist'] = __('Product already added', 'woolentor-pro') ;
}
...
$form_class_arr[] = 'wl-discount-placement--'. $config['discount_percent_placement'];
			...
			<tbody>
				...
				foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
					...
					if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
						...
						$desc = $_product->is_type('variation') ? $_product->get_description() : $_product->get_short_description();
						...
						?>
						<tr class="product woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
							<td>
									...
									<div class="woolentor-cart-product-content">
										<div class="woolentor-cart-product-content-left">
											...
											<?php
												foreach( $orders_arr_1 as $key => $item ){
													// Stock availability
													if( $key == 'stock_availability' && $config['stock_availability_placement'] != 'right' && $config['show_product_stock'] === 'yes' && !empty($availability['availability']) && !empty($availability['class']) ){
													...
													// SKU
													if( $key == 'sku' && $config['show_sku'] === 'yes' && $_product->get_sku() ){
													...
													if( $key == 'meta' && $config['show_meta_data'] === 'yes' ){
														echo Woolentor_Cart_Page::wl_get_formatted_cart_item_data( $cart_item ); // PHPCS: XSS ok.
													}
												}
											?>
										</div>
										
										<?php if( $config['qty_input_placement'] === 'after_title' ): ?>
										<div class="woolentor-cart-product-content-middle">
											<div class="product-quantity woolentor-product-quantity">
												<?php if( !empty($config['label_qty']) ){ 
													echo '<span>'. $config['label_qty'] .' </span>';
												} ?>
										...
										<div class="woolentor-cart-product-content-right">
											<div class="woolentor-product-price">
												...
												<?php if( $cart_product_raw_price != $product_raw_price ): ?>
												<div class="woolentor-product-price-old">
													...
													if( $discount_amount_percent && 
														$config['discount_percent_placement'] === 'with_price'
													){
											...
											<?php
												...
												if( $config['stock_availability_placement'] === 'right' && $config['show_product_stock'] === 'yes' && !empty($availability['availability']) && !empty($availability['class']) ){
													printf('<span class="woolentor-cart-product-stock %s">%s</span>',
														$availability['class'],
														$availability['availability']
													);
												...
												/* Showing the discount amount. */
												if( $discount_amount_percent && 
													$config['discount_percent_placement'] === 'separate_line'
												){
													printf('<div class="woolentor-cart-product-sale">%s</div>',
														$discount_percent_label
													);
										...
										<!-- Action buttons -->
										<?php if( $config['show_details_action'] === 'yes' || $config['show_remove_action'] === 'yes' || $config['show_compare_action'] === 'yes' || $config['show_wishlist_action'] === 'yes' ): ?>
										<div class="woolentor-cart-product-actions wl-style--<?php echo esc_attr($config['action_button_layout']) ?>">

											<?php
												// Description toggle action
												if( $config['show_details_action'] === 'yes' && $desc ): ?>
												<a href="#" class="woolentor-cart-product-details-toggle woolentor-cart-product-actions-btn" data-target="woolentor_<?php echo esc_attr($_product->get_id()); ?>"><?php echo wp_kses_post($config['label_show_product_details']); ?>
													<svg width="10" height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg">
														<path d="M1.50001 0.200195L5 3.70019L8.5 0.200195L9.9 0.900195L5 5.80019L0.100006 0.900195L1.50001 0.200195Z" fill="currentColor"></path>
													</svg>
												</a>
											<?php endif;
										...
										<?php if( $config['show_details_action'] === 'yes' && $desc ): ?>
											<div class="woolentor-cart-product-details-list" data-id="woolentor_<?php echo esc_attr($_product->get_id()); ?>" style="display: none;">
												<?php echo wp_kses_post($desc) ?>
											</div>
										<?php endif; ?>
										<?php endif; // action buttons ?>
									</div><!-- .woolentor-cart-product-actions -->
								</div>
							</td>
						</tr>
						<?php
