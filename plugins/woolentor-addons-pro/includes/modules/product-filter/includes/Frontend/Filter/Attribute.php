...
class Attribute {
    ...
    public function __construct( $settings = array(), $show_label = true, $group_item = false, $group_apply = '', $group_clear = '' ) {
        ...
        $attribute            = ( isset( $settings['filter_attribute'] ) ? wlpf_cast( $settings['filter_attribute'], 'key' ) : '' );
        $terms_include        = ( isset( $settings['filter_attribute_terms_include'] ) ? wlpf_cast( $settings['filter_attribute_terms_include'], 'text' ) : '' );
        $terms_exclude        = ( isset( $settings['filter_attribute_terms_exclude'] ) ? wlpf_cast( $settings['filter_attribute_terms_exclude'], 'text' ) : '' );
        ...
        if ( empty( $attribute ) || empty( $unique_id ) ) {
            return;
        }
    ...
    protected function get_classes() {
        ...
        $classes .= ' wlpf-filter-wrap wlpf-attribute-filter';
    ...
    protected function get_attributes( $terms_data, $selected_data ) {
        ...
        $attributes .= ' data-wlpf-attribute="' . esc_attr( $this->attribute ) . '"';
    ...
    protected function get_terms_ids( $parent = 0, $ids = array() ) {
        ...
        if ( is_array( $terms_ids ) && ! empty( $terms_ids ) ) {
            ...
            foreach ( $terms_ids as $term_id ) {
                ...
                $child_terms_ids = get_term_children( $term_id, $this->attribute );
                ...
                if ( true === $this->children_terms ) {
                    if ( 0 === $lap ) {
                        if ( ( empty( $include ) || in_array( $term_id, $include ) ) && ( empty( $exclude ) || ! in_array( $term_id, $exclude ) ) ) {
                            ...
                            $parent_terms_ids = wlpf_get_parent_terms_ids( $term_id, $this->attribute );
    ...
    protected function get_selected_data() {
        $data = \WLPF\Frontend\Selected::get_data( 'attribute' );
        $attr_filter = ( isset( $data['attr_filter'] ) ? wlpf_cast( $data['attr_filter'], 'array' ) : array() );
        $attribute = ( isset( $attr_filter[ $this->attribute ] ) ? wlpf_cast( $attr_filter[ $this->attribute ], 'array' ) : array() );

        $terms = ( isset( $attribute['terms'] ) ? wlpf_cast( $attribute['terms'], 'array' ) : array() );
        $terms_info = ( isset( $attribute['terms_info'] ) ? wlpf_cast( $attribute['terms_info'], 'array' ) : array() );
        ...
        if ( is_array( $terms_data ) && ! empty( $terms_data ) ) {
            ...
            foreach ( $terms_data as $term_data ) {
                ...
                if ( isset( $selected_data[ $id ] ) ) {
                    $list_content .= '<input class="wlpf-term-field" type="' . $this->field_type . '" name="wlpf-' . $this->attribute . '" value="' . $id . '" checked="checked">';
                } else {
                    $list_content .= '<input class="wlpf-term-field" type="' . $this->field_type . '" name="wlpf-' . $this->attribute . '" value="' . $id . '">';
                }
    ...
    protected function prepare_content() {
        ...
        $parent = ( ( $fixed_taxonomy === $this->attribute ) ? $fixed_taxonomy_term_id : 0 );
